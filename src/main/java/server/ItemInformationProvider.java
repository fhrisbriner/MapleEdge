...
public class ItemInformationProvider {
    ...
    */
    ...
    public int getSpellTraceCostt(int nItemLevel, int nReqLevel, boolean superiorEquip) {
        if(nItemLevel <= 0){
            return 0;
        }
        int baseMesoCost = (2 * nReqLevel / 10);
        double mesoMultiplier = 1.0;
        if(superiorEquip){
            mesoMultiplier = 1.0 * nItemLevel * .1;
            baseMesoCost = 1000000 * (nReqLevel / 10);;
        } else {
            mesoMultiplier += .1 * nItemLevel;
            if(nItemLevel >= 10) {
                mesoMultiplier *= 4;
            }
        }
        if((int) (baseMesoCost * mesoMultiplier) > 500000000) {
            return 500000000;
        }
        return (int) (baseMesoCost * mesoMultiplier);
    }
